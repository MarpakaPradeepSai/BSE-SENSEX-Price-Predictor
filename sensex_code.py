...
# Load the BSE SENSEX model
model_file = 'LSTM_SENSEX_5(568.19).h5'
try:
    model = load_model(model_file)
    print("BSE SENSEX model loaded successfully.")
except Exception as e:
    print(f"Error loading BSE SENSEX model: {e}")

# Function to get the stock data
def get_stock_data(ticker='^BSESN'):
...
# Set BSE SENSEX as the selected stock
stock = '^BSESN'
...
if st.button(f'Predict Next {num_days} Days BSE SENSEX Prices', key='forecast-button'):
    # Load stock data
    ...
    st.markdown(f"### Historical Data for BSE SENSEX")
